...
import { User, Mail, Phone, MapPin, Heart, Users, Clock, Shield, Edit2, CheckCircle, AlertCircle, LogOut, ChevronRight, Droplet, Activity, Award, Share2, Info } from 'lucide-react';
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 hover:scale-[1.02] transform">
              ...
              <div className="flex flex-col items-center text-center mb-6">
                ...
                <h2 className="text-xl font-bold text-gray-800 mt-4 hover:text-purple-600 transition-colors duration-300">Shreya Gupta</h2>
                <div className="flex flex-wrap items-center justify-center gap-2 mt-2">
                  <span className="text-xs bg-purple-100 text-purple-600 px-3 py-1.5 rounded-full font-semibold hover:bg-purple-200 transition-all duration-300 hover:scale-105">Premium Member</span>
                  <span className="text-xs bg-green-100 text-green-600 px-3 py-1.5 rounded-full font-semibold hover:bg-green-200 transition-all duration-300 hover:scale-105">Verified ✓</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[
                  { number: '12', label: 'ALERTS', color: 'from-purple-500 to-pink-500' },
                  { number: '6', label: 'CONTACTS', color: 'from-blue-500 to-purple-500' },
                  { number: '98', label: 'SAFETY', color: 'from-green-500 to-teal-500' }
                ].map((stat, index) => (
                  <div 
                    key={index}
                    className={`bg-gradient-to-br ${stat.color} rounded-xl p-3 text-center transform transition-all duration-300 hover:scale-110 hover:shadow-lg cursor-pointer`}
                  >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InfoRow icon={User} label="Full Name" value="Shreya Gupta" />
                <InfoRow icon={Mail} label="Email" value="admin@gmail.com" verified />
                <InfoRow icon={Phone} label="Phone" value="+91 9699 XXX XXX" verified />
                <InfoRow icon={MapPin} label="Address" value="Amravati, MH 444606" />
                <InfoRow icon={Droplet} label="Blood Group" value="A+ (Positive)" />
                <InfoRow icon={Activity} label="Medical Notes" value="No known allergies. Asthma inhaler in bag." />
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
              ...
              <div className="space-y-3">
                <EmergencyContact name="AAAA AAAA" relation="Sister" phone="+91 111-111-1111" status="Active" />
                <EmergencyContact name="BBBB BBBB" relation="Brother" phone="+91 222-222-2222" status="Active" />
                <EmergencyContact name="CCCC CCCC" relation="Friend" phone="+91 333-333-3333" status="Pending" />
              </div>
            </div>
            ...
            {/* Logout Button */}
            <button className="w-full py-3.5 bg-red-50 text-red-600 rounded-2xl text-sm font-semibold hover:bg-red-100 transition-all duration-300 flex items-center justify-center gap-2 border-2 border-red-200 hover:border-red-400 hover:scale-105 hover:shadow-lg transform">
...
const EmergencyContact = ({ name, relation, phone, status }) => (
  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-purple-50 transition-all duration-300 hover:scale-105 hover:shadow-md transform cursor-pointer">
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-sm font-bold text-gray-800 hover:text-purple-600 transition-colors duration-300">{name}</p>
    ...
    <div className="flex items-center gap-2">
      <span className={`text-[10px] px-3 py-1.5 rounded-full font-medium transition-all duration-300 hover:scale-110 ${
        status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700 animate-pulse'
      }`}>
        {status}
      </span>
